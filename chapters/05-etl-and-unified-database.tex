...
This chapter presents Sprint 1, focused on building the ETL foundation and the unified operational database. The goal was to bring data from different systems into one clean, consistent, and monitorable flow before using it in business modules.

\section{Sprint Backlog}
The sprint backlog for Sprint 1 was formalized as backlog items focused on ETL and unified database delivery:
...
\hline
S1-BL-01 & Connect to BioTime and EasyProject data sources so the ETL process can extract operational data. & High \\
\hline
S1-BL-02 & Build entity-based extraction pipelines for departments, employees, attendance, projects, and members to structure ingestion. & High \\
\hline
S1-BL-03 & Apply transformation and normalization rules to keep unified data clean and consistent. & High \\
\hline
S1-BL-04 & Load transformed records into the PostgreSQL target schema while preserving referential integrity. & High \\
\hline
S1-BL-05 & Monitor ETL execution status and errors in real time so failures can be detected early. & Medium \\
\hline
\end{tabular}
...
\section{Conclusion}
Sprint 1 delivered a robust and observable ETL foundation and produced the unified PostgreSQL database used by later modules. The resulting data layer ensures consistency, traceability, and operational readiness for authentication, HR, and project-management features.
