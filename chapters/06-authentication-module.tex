...
\begin{figure}[H]
	...
	\IfFileExists{diagrams/auth/auth-use-case-refinement.png}{%
		\makebox[\textwidth][c]{\includegraphics[width=1.08\textwidth,height=0.94\textheight,keepaspectratio]{diagrams/auth/auth-use-case-refinement.png}}
	}{%
...
Figure~\ref{fig:auth-use-case-refinement} summarizes the authentication workflow through three packages: Login, Registration, and Account Approval.

\clearpage
\subsection{Use Case Refinement Description}
This subsection gives a simple textual description of the use cases shown in the refined diagram. It explains the role of each package and clarifies the main actions performed during login, registration, and account approval. Table~\ref{tab:auth-use-case-description} presents these use cases in an organized form.
...
\begin{table}[H]
	...
	\makebox[\textwidth][c]{%
		\begin{tabular}{|>{\raggedright\arraybackslash}p{2.8cm}|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}p{7.7cm}|}
			\hline
			\rowcolor{gray!15}
			\textbf{Package} & \textbf{Use Case} & \textbf{Simple Explanation} \\
			\hline
			\textbf{Login} & Login & The user enters their email and password. The system checks the account before giving access. \\
			\hline
			\textbf{Login} & Verify Credentials & The system checks if the email and password are correct and belong to a valid account. \\
			\hline
			\textbf{Login} & Handle Invalid Credentials & If the email or password is wrong, the system refuses access and shows an error message. \\
			\hline
			\textbf{Login} & Handle Unapproved Account & If the account is still pending, the system blocks login until the administrator approves it. \\
			\hline
			\textbf{Registration} & Register User & A new HR or Project Manager creates an account using the registration form. \\
			\hline
			\textbf{Registration} & Fill Registration Form & The user enters their information, email, password, and role. The account is saved as pending. \\
			\hline
			\textbf{Account Approval} & Review Pending Accounts & The administrator checks the list of new accounts waiting for approval. \\
			\hline
			\textbf{Account Approval} & Approve Account & If the request is valid, the administrator approves the account so the user can log in. \\
			\hline
			\textbf{Account Approval} & Reject Account & If the request is not valid, the administrator rejects it and the user cannot access the platform. \\
			\hline
		\end{tabular}%
	}
	\caption{Simple explanation of the authentication use cases}
