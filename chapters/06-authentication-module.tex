...
This chapter presents Sprint 2, focused on Authentication and Access Control. These features control secure access and verify user identity. They manage registration, login, administrator approval, and an extra OTP email check for Admin login.
...
During Sprint 2, we selected and implemented the following backlog items:
...
\begin{table}[H]
	...
	\begin{tabular}{|l|>{\raggedright\arraybackslash}p{9.2cm}|l|}
		...
		\hline
		S2-BL-01 & Implement login using email and password credentials. & High \\
		\hline
		S2-BL-02 & Implement account registration for HR and Project Manager roles. & High \\
		\hline
		S2-BL-03 & Implement administrator approval for newly registered user accounts. & High \\
		\hline
		S2-BL-04 & Add email-based OTP verification during Administrator login. & High \\
		\hline
		S2-BL-05 & Send an email notification after account approval. & Medium \\
		\hline
		S2-BL-06 & Block application access for unapproved user accounts. & High \\
		\hline
	\end{tabular}
...
\begin{figure}[H]
	...
	\caption{Refined use case diagram for authentication and access control}
	\label{fig:auth-use-case-refinement}
\end{figure}

Figure~\ref{fig:auth-use-case-refinement} summarizes the authentication workflow through three packages: Login, Registration, and Account Approval. To make the diagram easier to understand, Table~\ref{tab:auth-use-case-description} explains each use case shown in the diagram.
...
\begin{table}[H]
	\centering
	\renewcommand{\arraystretch}{1.2}
	\begin{tabular}{|>{\raggedright\arraybackslash}p{3.1cm}|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{7.5cm}|}
		\hline
		\textbf{Package} & \textbf{Use Case} & \textbf{Explanation} \\
		\hline
		Login & Login & The user starts the authentication process by entering their email and password. This is the main use case of the login package and it triggers credential verification before access can be granted. \\
		\hline
		Login & Verify Credentials & This use case is included in the login process. The system checks whether the submitted email and password match an existing account and whether the account information is valid. \\
		\hline
		Login & Handle Invalid Credentials & This use case extends the login process when the entered email or password is incorrect. The system blocks access and displays an error message without revealing sensitive details. \\
		\hline
		Login & Handle Unapproved Account & This use case extends the login process when the credentials are correct but the account has not yet been approved by the administrator. The system refuses access until approval is completed. \\
		\hline
		Registration & Register User & A new HR or Project Manager starts account creation. This use case represents the global registration action and includes filling out the registration form. \\
		\hline
		Registration & Fill Registration Form & This use case is included in registration. The user enters required information such as identity, email, password, and requested role. After submission, the account is saved with a pending status. \\
		\hline
		Account Approval & Review Pending Accounts & The administrator opens the pending-account list to inspect registration requests. This use case is the entry point of the approval workflow and includes either approving or rejecting an account. \\
		\hline
		Account Approval & Approve Account & This use case is included in reviewing pending accounts. If the request is valid, the administrator approves it, changing the account status so the user can log in and access authorized features. \\
		\hline
		Account Approval & Reject Account & This use case is included in reviewing pending accounts. If the request is invalid or should not be accepted, the administrator rejects it and the user remains unable to access the platform. \\
		\hline
	\end{tabular}
	\caption{Textual explanation of the authentication use cases shown in the diagram}
	\label{tab:auth-use-case-description}
	{\footnotesize\textit{Source: Author's work.}}
\end{table}
...
The following security controls were applied in Authentication and Access Control:
...
\begin{table}[H]
	...
	\begin{tabular}{|l|>{\raggedright\arraybackslash}p{9.8cm}|}
		...
		\hline
		Approval-gated access & Users cannot access business features unless account status is approved by the administrator. \\
		\hline
		Admin OTP verification & Admin users must validate a one-time password sent by email before login is completed. \\
		\hline
	\end{tabular}
	\caption{Authentication security controls}
	\label{tab:authentication-security-controls}
	{\footnotesize\textit{Source: Author's work.}}
\end{table}

\section{Conclusion}
Authentication and Access Control ensures secure and controlled access to the application. With administrator approval, automatic email notification, and OTP verification for Admin users, the system adds an extra layer of protection for sensitive access.
