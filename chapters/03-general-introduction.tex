...
\centering
	...
	\begin{tabular}{|l|>{\raggedright\arraybackslash}p{10cm}|}
		...
		\hline
		\textbf{Internship Duration} & 14 weeks (approximately 3.5 months) \\
		\hline
...
{\footnotesize\textit{Source: Adapted from OneTech Business Solutions and OneTech Group public information.}}
\end{table}

\subsection{Organizational Chart}
Figure~\ref{fig:group-management-structure} provides a structured view of the company's group management hierarchy. It helps clarify the organizational levels and shows the position of OTBS within the broader OneTech Group structure.
...
\begin{figure}[H]
	...
	\IfFileExists{assets/general/easyproject-definition.jpg}{%
		\includegraphics[width=0.50\textwidth,height=0.42\textheight,keepaspectratio]{assets/general/easyproject-definition.jpg}
	}{%
		\fbox{\parbox{0.9\textwidth}{
			\centering
			\vspace{1.6cm}
...
\centering
	...
	\begin{tabular}{|>{\raggedright\arraybackslash}p{3.3cm}|>{\centering\arraybackslash}p{2.1cm}|>{\raggedright\arraybackslash}p{8.8cm}|}
		\hline
		\rowcolor{gray!20}
		\textbf{Category} & \textbf{ID} & \textbf{Description} \\
		\hline
		\textbf{Administrator} & FR-01 & Create and manage user accounts with their assigned roles: Admin, HR, or Project Manager. \\
		\cline{2-3}
		 & FR-02 & Assign projects to Project Managers. \\
		\hline
		\textbf{Human Resources (HR)} & FR-03 & View all employees across the organization. \\
		\cline{2-3}
		 & FR-04 & View attendance records for all employees. \\
		\cline{2-3}
		 & FR-05 & View attendance leave information. \\
		\hline
		\textbf{Project Manager} & FR-06 & View only projects assigned by the Administrator. \\
		\cline{2-3}
		 & FR-07 & View members belonging to assigned projects. \\
		\cline{2-3}
		 & FR-08 & View attendance records only for members of assigned projects. \\
		\hline
		\textbf{ETL \& System} & FR-09 & Extract attendance and HR data from BioTime and EasyProject on schedule. \\
		\cline{2-3}
		 & FR-10 & Resolve duplicate records and identifier mismatches. \\
		\cline{2-3}
		 & FR-11 & Store transformed and unified data in the main PostgreSQL database through \textbf{SQLAlchemy}. \\
		\cline{2-3}
		 & FR-12 & Provide synchronization capabilities for keeping source data updated. \\
		\hline
	\end{tabular}
...
\centering
	\renewcommand{\arraystretch}{1.25}
	\setlength{\tabcolsep}{6pt}
	\begin{tabular}{|>{\raggedright\arraybackslash}p{3.0cm}|>{\centering\arraybackslash}p{2.2cm}|>{\raggedright\arraybackslash}p{9.0cm}|}
		\hline
		\rowcolor{gray!20}
		\textbf{Category} & \textbf{Requirement ID} & \textbf{Description} \\
		\hline
		\textbf{Performance} & NFR-01 & API endpoints shall provide fast response times for standard queries on large employee datasets. \\
		\hline
		\textbf{Security} & NFR-02 & All API routes shall be protected by JWT-based authentication. Passwords shall be hashed using bcrypt. \\
		\cline{2-3}
		 & NFR-03 & Role-based access control (RBAC) shall enforce permission boundaries. \\
		\hline
		\textbf{Reliability} & NFR-04 & ETL shall handle source failures and log errors clearly. \\
		\hline
		\textbf{Data Integrity} & NFR-05 & Duplicate attendance records shall be prevented through ETL deduplication and unique constraints. \\
		\hline
		\textbf{Scalability} & NFR-06 & The system shall remain easy to extend when new data is added. \\
		\hline
		\textbf{Usability} & NFR-07 & The React interface shall be responsive on desktop and tablet screens. \\
		\hline
		\textbf{Maintainability} & NFR-08 & Database migrations shall be managed with Alembic. \\
		\hline
		\textbf{Portability} & NFR-09 & The application shall be Docker-compatible for deployment flexibility. \\
		\hline
	\end{tabular}
...
\centering
	...
	\begin{tabular}{|>{\centering\arraybackslash}m{1.8cm}|>{\centering\arraybackslash}m{3.1cm}|>{\raggedright\arraybackslash}p{8cm}|}
		...
		\hline
		Sprint 0 & 2 weeks & Study the source systems, define the required data scope, and prepare the main conception artifacts for system analysis and design (Chapter 2). \\
		\hline
		Sprint 1 & 2 weeks & Implement the ETL pipelines and populate the centralized PostgreSQL database for the ETL and unified database phase (Chapter 3). \\
		\hline
		Sprint 2 & 2 weeks & Implement login, registration, administrator approval, and role-based access control for the authentication module (Chapter 4). \\
		\hline
		Sprint 3 & 2 weeks & Deliver user management, project assignment, and synchronization settings features for the administration module (Chapter 5). \\
		\hline
		Sprint 4 & 2 weeks & Implement employee management, attendance follow-up, and leave handling interfaces for the HR functional modules (Chapter 6). \\
		\hline
		Sprint 5 & 2 weeks & Deliver project, member, and attendance monitoring features for project managers through the Project Manager modules (Chapter 7). \\
		\hline
		Sprint 6 & 2 weeks & Integrate the RH Agent and finalize the intelligent assistance workflow for the RH Agent integration phase (Chapter 8). \\
		\hline
	\end{tabular}
\caption{Sprint plan}
...
\begin{tabular}{|>{\centering\arraybackslash}p{1.8cm}|>{\raggedright\arraybackslash}p{3.0cm}|>{\raggedright\arraybackslash}p{5.0cm}|>{\raggedright\arraybackslash}p{5.0cm}|}
	...
	\hline
\end{tabular}
\caption{Technology stack and rationale, part 1}
\label{tab:technology-stack-part1}
{\footnotesize\textit{Source: Author's work, based on project technology choices.}}
\end{table}
...
\begin{tabular}{|>{\centering\arraybackslash}p{1.8cm}|>{\raggedright\arraybackslash}p{3.0cm}|>{\raggedright\arraybackslash}p{5.0cm}|>{\raggedright\arraybackslash}p{5.0cm}|}
	...
	\hline
\end{tabular}
\caption{Technology stack and rationale, part 2}
\label{tab:technology-stack-part2}
{\footnotesize\textit{Source: Author's work, based on project technology choices.}}
\end{table}
\normalsize
